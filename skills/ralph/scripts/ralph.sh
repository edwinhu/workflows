#!/usr/bin/env bash
# ralph.sh -- an unattended agent loop whose entire memory is one append-only journal.
#
...
usage() {
  cat >&2 <<'EOF'
ralph.sh -- an unattended loop whose only memory is one append-only journal.

  ralph.sh run    --journal J --check CMD --prompt-file F [--gate CMD] [--runner R]
                  [--model M] [--max-iters N] [--sleep S] [--stall-after K]
  ralph.sh append --journal J '{"kind":"progress","key":"..."}'
  ralph.sh floors --journal J
  ralph.sh status --journal J
  ralph.sh tail   --journal J [-n N] [-f]
  ralph.sh stop   --journal J [--why TEXT]
...
journal_append() {
  ...
  case "$rec" in
    *$'\n'*|*$'\r'*)
      printf 'ralph: refused: a record must be ONE line; %s\n' 'not split across two' >&2; return 2 ;;
  esac
  ...
  if [ "${n:-0}" -gt "$((ATOMIC_BOUND - 2))" ]; then
    printf 'ralph: refused: record is %s bytes, over the %s-byte single-write bound; not split, not truncated\n' \
      "$n" "$ATOMIC_BOUND" >&2
    return 2
  fi
  jq -e -s 'length == 1 and (.[0] | type == "object")' >/dev/null 2>&1 <<<"$rec" || {
    printf 'ralph: refused: not one JSON object on one line\n' >&2; return 2; }
...
check_agent_record() {
  ...
  case "$kind" in
    ...
    *)
      printf 'ralph: refused: kind %s is not yours to write; the agent may append [%s], the loop owns [%s]\n' \
  ...
  if [ "$kind" = floor ]; then
    jq -e -s '.[0].key | (type == "string" or type == "number") and (tostring | test("[^[:space:]]"))' \
      >/dev/null 2>&1 <<<"$rec" || {
      printf 'ralph: refused: a floor needs a non-empty key; without one the reader drops it and the key is re-diagnosed forever\n' >&2
      return 2; }
  fi
  return 0
}
...
case "$sub" in
  ...
  stop)   cmd_stop   "$@" ;;
  -h|--help|help) usage ;;
  *) printf 'ralph: unknown subcommand: %s\n' "$sub" >&2; usage ;;
esac
exit $?
