...
- **Judgement** — no checker exists at the rule's conventional name, and a grader agent judges it. Nothing declares that: the runner asks the filesystem, so a rule cannot claim to be mechanised while its checker is absent. **Getting the corpus INTO that grader is the hard part, and two of the three obvious routes do not work.** A `` !`cmd` `` in an agent definition is dead text. And `skills:` frontmatter makes a skill INVOCABLE — it adds the name and description to the agent's skill listing — it does **not** preload a line of it: measured 2026-09-15, `workshop-reviewer` carried `skills: [typst:typst]` and answered NOT PRESENT when asked for the index, under both `claude -p --agent` and the farm-out proxy. So a grader reaches the corpus by exactly one of:
  ...
  Writing "the index in your context" into a grader that has neither the `Skill` tool nor `refs` tells it to grade against something it does not have. That shipped here for a day.
