 ...
 * does not.
 *
 *   bun test ${CLAUDE_PLUGIN_ROOT}/skills/work/scripts/plan-lint.test.ts
...
// ------------------------------------------------- the rules, through work-dispatch.sh

import { execFileSync } from 'node:child_process'
import { mkdtempSync, rmSync, writeFileSync, mkdirSync, existsSync, chmodSync } from 'node:fs'

/**
 * work-dispatch.sh EXECUTES every task's `redCommand` before dispatching and refuses one that
...
test('a cycle is a hard error at the gate — critical, which work-dispatch.sh refuses', () => {
  const p = base({
    tasks: [
      task({ id: 'T1', writablePaths: ['src/1/'], dependsOn: ['T2'] }),
      task({ id: 'T2', writablePaths: ['src/2/'], dependsOn: ['T1'] }),
...
// ------------------------------------------------- the same shape, printed by work-dispatch.sh
...
test('work-dispatch.sh prints a parallel set as one wave', () => {
...
test('work-dispatch.sh prints a diamond with its middle wave paired', () => {
...
test('work-dispatch.sh refuses a cyclic plan and prints no layering', () => {
...
test('a bare script name in prose is a noun, not a stated command', () => {
  const md = [
    ...
    '- The dispatcher `work-dispatch.sh` and `workflows/work.js` are unchanged.',
  ].join('\n')
  expect(rules(parseMarkdown(md))).not.toContain('prose-command')
})
