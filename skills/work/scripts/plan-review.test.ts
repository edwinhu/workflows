 ...
 *
 *   bun test ${CLAUDE_PLUGIN_ROOT}/skills/work/scripts/plan-review.test.ts
 */
...
test('deletion:scripts — work-result.sh no longer names the plan channel', () => {
  expect(readFileSync(join(SCRIPTS, 'work-result.sh'), 'utf8')).not.toContain('planThatFlagged')
})
