// The two wirings between the prose engine and check-all.py that went stale silently at v6.0.0,
...
// it, and a SEVERITY="hard" constraint was filed under `passed`. cc-probe finds both across the
...
function checkAllLabels(): string[] {
  ...
  for (const skill of readdirSync(join(REPO, "skills"))) {
    const refs = join(REPO, "skills", skill, "references");
    if (!existsSync(refs)) continue;
    for (const f of readdirSync(refs)) {
      if (f.endsWith(".py")) labels.push(`skills/${skill}/references/${f.slice(0, -3)}`);
    }
  }
  return labels;
}
...
describe("PROSE_ENGINE_PREFIXES suppresses live check-all entries and only those", () => {
  ...
  // from skills/writing-{general,legal,econ}/references/ to skills/writing/references/.
  ...
  const STRUCTURAL = [
    "skills/writing/references/writing-topic-sentences",
    "skills/writing/references/writing-anchored-numbers",
    "skills/writing/references/writing-outline-sync",
    "skills/writing/references/writing-shortjournal",
  ];
...
describe("writing-no-bold-lead resolves the engine it delegates to", () => {
  ...
  const MODULE = join(REPO, "skills", "writing", "references", "writing-no-bold-lead.py");
  ...
  test("an unreachable engine raises rather than returning a clean pass", () => {
    ...
    // checker that never ran. check-all.py files a raised exception under `errors` instead.
    const proc = Bun.spawnSync([
      "python3", "-c",
